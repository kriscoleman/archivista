...
// predicateTypes implements the predicateTypes query with custom string-based cursor pagination.
func (r *queryResolver) predicateTypes(ctx context.Context, after *PredicateTypeCursor, first *int, before *PredicateTypeCursor, last *int, where *PredicateTypeWhereInput) (*PredicateTypeConnection, error) {
	...
	for i, pt := range pagePredicateTypes {
		edges[i] = &PredicateTypeEdge{
			Node:   pt,
			Cursor: PredicateTypeCursor{Value: pt.Value},
		}
	}

	// Build page info
	var startCursor, endCursor *PredicateTypeCursor
	if len(edges) > 0 {
		startCursor = &edges[0].Cursor
		endCursor = &edges[len(edges)-1].Cursor
	}

	pageInfo := PredicateTypePageInfo{
